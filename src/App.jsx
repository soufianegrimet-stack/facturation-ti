...
const INIT_INVOICES = [
  {
    ...
    status: "paid", lignes: [{ desc: "Transport Rabat/Casablanca", qte: 3, pu: 4500 }], notes: "", tva: true
  },
  {
    id: "FAC-2024-002", clientId: "C002", date: "2024-02-01", echeance: "2024-03-01",
    status: "sent", lignes: [{ desc: "Transport maritime Casablanca/Marseille", qte: 1, pu: 18000 }], notes: "", tva: true
  },
];
      ...
      <main style={S.main}>
        ...
        {isFactures && !editingInvoice && view === "factures" && (
          <Factures invoices={invoices} clients={clients} calcTotal={calcTotal}
            onNew={() => setEditingInvoice({ id: nextInvoiceId(), clientId: "", date: today(), echeance: "", status: "draft", lignes: [{ desc: "", qte: 1, pu: 0 }], notes: "", tva: true })}
      ...
      <div id="invoice-print" style={S.invPrint}>
          ...
          <tbody>
            {inv.lignes.map((l, i) => (
              <tr key={i} style={{ background: i%2===0 ? "#fff" : "#f8fafc" }}>
                ...
                <td style={{ padding:"10px 14px", fontSize:13, color:"#334155", textAlign:"right", borderBottom:"1px solid #e2e8f0" }}>{formatMoney(Number(l.pu)||0)}</td>
                <td style={{ padding:"10px 14px", fontSize:13, color:"#334155", textAlign:"right", fontWeight:700, borderBottom:"1px solid #e2e8f0" }}>{formatMoney((Number(l.qte)||0)*(Number(l.pu)||0))}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:5, marginBottom:16 }}>
          <div style={{ display:"flex", gap:60, color:"#64748b", fontSize:13 }}>
            <span>Total HT</span><span>{formatMoney(ht)}</span>
          </div>
          {inv.tva ? (
            <div style={{ display:"flex", gap:60, color:"#64748b", fontSize:13 }}>
              <span>TVA (20%)</span><span>{formatMoney(tva)}</span>
            </div>
          ) : (
          ...
          <div style={{ display:"flex", gap:60, color:"#fff", fontSize:16, fontWeight:800, background:"#0f172a", padding:"10px 16px", borderRadius:6, marginTop:4 }}>
            <span>{inv.tva ? "TOTAL TTC" : "TOTAL HT"}</span><span>{formatMoney(ttc)} MAD</span>
          </div>
        </div>
      ...
      <div style={S.formGrid}>
        ...
        <div style={S.formCard}>
          ...
          <div style={{ marginTop:16, borderTop:"2px solid #e2e8f0", paddingTop:12 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, margin:"12px 0 4px", padding:"10px 14px", background:form.tva?"#eff6ff":"#f8fafc", borderRadius:8, border:"1.5px solid "+(form.tva?"#3b82f6":"#e2e8f0") }}>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:18, fontWeight:800, color:"#0f172a", borderTop:"2px solid #0f172a", paddingTop:8, marginTop:4 }}>
              <span>{form.tva ? "Total TTC" : "Total HT"}</span><span>{formatMoney(form.tva ? ht*1.2 : ht)}</span>
            </div>
          </div>
