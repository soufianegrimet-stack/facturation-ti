...
const STATUS = {
  ...
  compensated:  { label: "Compensé",   color: "#8b5cf6", bg: "#f5f3ff" },
};
...
function getStatus(inv) {
  ...
  if (inv.status === "compensated") return "compensated";
  if (inv.status === "cancelled")   return "cancelled";
  if (inv.status === "draft")       return "draft";
  if (inv.echeance && new Date(inv.echeance) < new Date() && inv.status !== "paid" && inv.status !== "compensated" && inv.status !== "cancelled") return "overdue";
...
function Dashboard({ invoices, clients, calcTotal, onViewInvoice, onGoto, currentUser }) {
  const paidInvs  = invoices.filter(i => getStatus(i) === "paid" || getStatus(i) === "compensated");
  const attente   = invoices.filter(i => getStatus(i) === "sent" || getStatus(i) === "overdue").reduce((s, i) => s + calcTotal(i), 0);
    ...
    <div style={S.page}>
      ...
      {(() => {
        ...
        const urgents = invoices.filter(i => {
          if (getStatus(i) === "paid" || getStatus(i) === "draft" || getStatus(i) === "compensated" || getStatus(i) === "cancelled") return false;
...
<div style="margin-bottom:16px">
  ...
  <table style="width:100%;border-collapse:collapse;border:1px solid #cbd5e1;border-radius:6px;overflow:hidden;font-size:10px">
    ...
    const stPrint = STATUS[getStatus(inv)] || { label: inv.status || "—", color: "#64748b", bg: "#f1f5f9" };
    const statusStyle = `background:${stPrint.bg};color:${stPrint.color};padding:4px 14px;border-radius:20px;font-weight:800;font-size:10px;display:inline-block;letter-spacing:0.5px`;
  ...
  <div style="text-align:right;padding-left:20px;min-width:200px">
    ...
    <table style="width:auto;margin-left:auto;font-size:10px">
      ...
      <tr>
        ...
        <td style="text-align:right"><span style="${statusStyle}">${stPrint.label}</span></td>
      </tr>
    </table>
  </div>
</div>
